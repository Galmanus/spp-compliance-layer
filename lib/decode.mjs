...
export function decodeEvent(ev) {
  ...
  // Layer-3 attestation proves a chain over. asp-membership lib.rs:52.
  if (name === "LeafAddedEvent" || name === "leaf_added") {
    return {
      ...base,
      kind: "asp_root",
      leaf: readU256(topics[1] ?? value?.leaf),
      leafIndex: readU32(value?.index ?? value?.[1]),
      root: readU256(value?.root ?? value?.[2]),
    };
  }
