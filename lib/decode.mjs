...
export function decodeEvent(ev) {
  ...
  // Bytes) — pool.rs:193. `commitment` is a topic; `index` and `encrypted_output`
  // live in the value MAP, so they must be read with mapField, exactly as the
  // asp_root branch does. Reading them as plain object properties
  // (value.index / value[0]) always yields null on the real JSON-XDR map shape,
  // which would silently null every leafIndex and ciphertext and break note
  // discovery. Verified against the deployed pool's event struct.
  if (name === "NewCommitmentEvent" || name === "new_commitment") {
    return {
      ...
      encryptedOutput: readBytes(field("encrypted_output")),
    };
  }
