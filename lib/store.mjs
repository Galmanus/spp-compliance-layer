...
export class Store {
  ...
  /**
   * Events in [fromLedger, toLedger], ledger-ascending, keyed for cursor paging.
   * This is what a bootnode-compatible `getEvents` serves: the raw events the
   * RPC has forgotten, in the same order and with the same paging discipline the
   * wallet's own sync loop expects. Each row carries enough to reconstruct the
   * on-chain event shape (kind, ledger, event_id, and the decoded fields);
   * ordering is by (ledger, event_id) so a cursor is a total order, not a
   * ledger-collision guess.
   */
