...
export async function ingestPool(store, pool, { fromLedger, pageLimit = 200, maxPages = 10_000, onPage } = {}) {
  ...
  for (;;) {
    ...
    // On cursor pages `startLedger` is null (the cursor carries the position),
    // so the coverage this page adds runs from just after the last ledger the
    // previous page confirmed. Without this, cursor pages would record no
    // coverage at all and a multi-page pool would report spurious gaps for every
    // range past the first page — false "incomplete", never a hidden real gap,
    // but wrong. getEvents-by-cursor is contiguous, so this claim is sound.
    const coverFrom = startLedger ?? report.scannedThrough + 1;
    report.rowsWritten += store.ingestBatch(pool, {
      commitments,
      nullifiers,
      fromLedger: coverFrom,
      toLedger: Math.max(coverFrom, scannedTo),
    });
