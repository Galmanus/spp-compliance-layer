...
async function call(method, params, { url = DEFAULT_RPC, timeoutMs = 30_000 } = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      // Some RPCs surface the retention refusal with a non-200 status and the
      // real message in the body. Fold that message into the error so
      // isOutsideRetention can still classify it — otherwise the one refusal the
      // whole project hinges on would be masked as a generic transport failure.
      let detail = "";
      try {
        const errBody = await res.json();
        if (errBody?.error?.message) detail = `: ${errBody.error.message}`;
      } catch {
        /* body was not JSON; keep the bare status */
      }
      throw new RpcError(`${method}: HTTP ${res.status}${detail}`, { http: res.status });
    }
...
export async function measureRetention(opts) {
  ...
  const { sequence, protocolVersion } = await getLatestLedger(opts);
  ...
  return {
    oldestLedger: r.oldestLedger,
    latestLedger: r.latestLedger,
    latestLedgerCloseTime: r.latestLedgerCloseTime,
    oldestLedgerCloseTime: r.oldestLedgerCloseTime,
    protocolVersion,
    ledgers,
