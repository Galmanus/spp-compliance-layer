...
same transaction. To our knowledge this is the first transparent
(trusted-setup-free) post-quantum proof verified on-chain on Stellar in a
privacy-compliance context, and the first to gate on-chain state in one; the
claim is checkable, given the SDF's own statement that no drop-in post-quantum
replacement for pairing SNARKs exists. On Stellar testnet, at the gated contract
