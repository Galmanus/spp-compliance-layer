...
This attestation is not only checkable off-chain. It is verified \emph{on-chain},
inside a Soroban contract, in a single Stellar transaction --- over the real
15-leaf history. To our knowledge this is the first transparent
(trusted-setup-free) post-quantum proof verified on-chain on Stellar in a
privacy-compliance context; the claim is checkable, given the SDF's own statement
that no drop-in post-quantum replacement for pairing SNARKs exists. On Stellar
testnet the honest proof returns \code{true}
(tx \code{96110dc1\dots}) and a tampered public root returns \code{false}
(tx \code{ee36ca3d\dots}), at contract
\code{CCWNNU4K3LWEPFRI7HLXV2WA2CV7C4Z7MFMWP7I7JZWESLTLPNHPCWRG}. It fits: the
optimized wasm is 112\,KB (under Soroban's 128\,KB code limit), and at 40 FRI
queries the verification costs 260M instructions --- 65\% of one transaction's
CPU budget, measured on the metered host, not estimated.
...
at a stated and lower security level. Reaching 62 quantum bits on-chain would
require splitting the verification across transactions or a larger extension
field; neither is claimed as done, and the on-chain figure is not presented as
production-grade.
